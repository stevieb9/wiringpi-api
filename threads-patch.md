...
> **⛔ PARKED — back burner (2026-06-03).** All wiringPi-threads work is deprioritized behind the ISR migration (`isr-migration.md` / `isr-examples.md`). Don't start these tasks until the ISR work lands and the hold is lifted. Usage examples are in `threads-examples.md` (also parked).
> **NEXT ACTION (when resumed):** V1 — delete the dead, unexposed `initThread` + `thread_callback_sv` (API.xs) so no ithread-hostile thread globals remain.
...
|----|------|---------|----------|--------|
| V1 | **Remove dead C-thread code (F20).** Delete `initThread` (C fn API.xs:231-259 + XSUB API.xs:493-494), the file-global `thread_callback_sv`, and the GCC-nested `PI_THREAD(myThread)`. It is unexposed (no Perl wrapper/export/POD — verified) and never used; removing it also eliminates ithread-hostile global SV state. (Distinct from `isr-migration.md` V4, which removes the *interrupt* dispatcher/`mine`.) | XS parse + `perl -c -Ilib lib/WiringPi/API.pm` + grep that `initThread`/`thread_callback_sv`/`PI_THREAD(` are gone | parses; no thread-global residue | ⏳ |
