...
|----|------|---------|----------|--------|
| _(none pending — migration complete)_ | | | | |
...
`docs/interrupt-examples.md`, scenario 9. An ithread-based equivalent — shared vars
instead of a pipe — is in `docs/threads-examples.md`, parked.)
...
`docs/interrupt-examples.md` scenario 8 (manual fork), which stays as the under-the-hood
...
- ✅ RESOLVED in **V7**: the `EDGE_RISING` vs `INT_EDGE_*` naming mismatch (raised in V2). The design-section examples here now use `INT_EDGE_RISING`; `docs/interrupt-examples.md` already used `INT_EDGE_*`; the POD documents the `INT_EDGE_*` constants. No `EDGE_*` aliases added — `INT_EDGE_*` (wiringPi's own names) is canonical.
...
- **Requiring threaded Perl** — works everywhere; background handling uses `fork`. ithread-based concurrency is parked (`threads-patch.md` / `docs/threads-examples.md`).
- **`initThread` / general threads / ithreads** — separate concern, **parked**; see `threads-patch.md` and `docs/threads-examples.md`.
