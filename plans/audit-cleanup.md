...
> **NEXT ACTION:** None — **audit complete and closed.** All V tasks (V1, V3–V7) done; backlog B1–B10 done; findings F1–F10 all resolved. B4's residual non-blocking-`read()` rewrite is **CLOSED — won't-do** — full assessment in `plans/B4.md`.
> **B4 DISPOSITION:** CLOSED — won't-do (author decision, 2026-06-11). Not an open item. Rationale: effectively unreachable in this GPIO library's domain (returns are tens of bytes; a stall needs >4KB, and is only *noticeable* >256KB pipe capacity), the worst case is a brief delay (no corruption — B10 fixed the only corruption facet), and the fix is the only medium-risk change left in a solid IPC path. Reopen only on a concrete need to stream large values through `{results}`; `plans/B4.md` has the implementation + test plan if so.
> **LAST SESSION:** 2026-06-11 — Audit closed out. Completed backlog B1–B10 (incl. B10, the shared-channel framing-desync guard discovered during the B4 analysis, with t/88) and wrote the B4 deferral assessment (`plans/B4.md`). Suite green at 142 tests.
...
B4: ✅ CLOSED — WON'T-DO (2026-06-11; doc fix done 2026-06-10): `BackgroundInterrupt`/`Worker` `read()`/`value()` assume the whole length-framed record is buffered once `select()` reports readable, then do a blocking `_read_exact`. True only while the payload stays under `PIPE_BUF` (4096B); a larger user return value can block the "non-blocking" drain. **Done (option (a), the doc fix):** corrected the misleading "won't block" inline comments in `BackgroundInterrupt::read()` and `Worker::value()` (now note the ~4KB limit, tagged `(B4)`), added a "Size limit" caveat to the `results`/`shared` POD in `API.pm`, regenerated `README.md` and synced `docs/pod.md`. **Decision (2026-06-11): the full non-blocking rewrite (option (b)) is CLOSED as won't-do** — see `## Explicitly NOT doing` and the full written assessment in `plans/B4.md` (facts, block mechanism, use-case analysis, cost/risk, options). Summary: effectively unreachable in this library's domain; worst case is a brief delay (no corruption — the shared-channel corruption facet was split out and fixed as B10); the rewrite is the only medium-risk change left. Reopen only on a concrete large-`{results}` use case; `plans/B4.md` has the implementation + test plan if so.
...
B10: ✅ RESOLVED (2026-06-11; discovered during the B4 analysis): the worker `{shared}` channel writer (`$val_w`) is non-blocking and ignored `syswrite`'s return, so a value whose framed record exceeds `PIPE_BUF` (4096B) could be **partially** written and **desync** `value()`'s length-framing — yielding a corrupted/garbage value or a stale misaligned stream (worse than B4's results-channel *delay*, because B4's `$res_w` is a blocking write that always emits whole records). **Resolution (the "cheap shared-writer guard" option, author approved):** the writer now skips frames `> PIPE_BUF` entirely (`if ($val_w && length($frame) <= PIPE_BUF)`), so oversized shared values are dropped cleanly — consistent with the channel's lossy contract. `{results}` is unchanged and still delivers large values intact. POD/comments updated (the `shared => 1` caveat now says oversized values are dropped, not "can block"); `README.md` regenerated + `docs/pod.md` synced. Added `t/88-shared_channel_framing.t` (11 assertions, all off-board): drop-vs-keep, exact `PIPE_BUF` boundary (frame == limit kept, limit+1 dropped), anti-desync sentinel + lossy-latest, and channel-scope (`results` delivers large intact; both-channels diverge correctly). Verified the tests catch the bug — 4 fail against the un-guarded writer. Full suite 142 pass. `Changes` entry added.
...
- B4's non-blocking `read()` rewrite (`O_NONBLOCK` + partial-record buffering on the `{results}` channel) — CLOSED won't-do (2026-06-11): the >4KB block is effectively unreachable in this library's domain and its worst case is a brief delay (no corruption — B10 fixed that facet); the rewrite is the only medium-risk change left in a solid IPC path. The 4KB limit is documented. Reopen only on a concrete large-`{results}` use case — full assessment + test plan in `plans/B4.md`.
