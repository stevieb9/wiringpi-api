...
> **NEXT ACTION:** Only B4's deferred real fix remains (see REVISIT below). All V tasks (V1, V3–V7) and backlog B1–B10 are done; B4 is documented + deferred.
> **REVISIT AT END:** B4 — the real non-blocking + partial-buffering drain for the **results** channel was deferred; for now only the `PIPE_BUF` limit is documented, so a `>4KB` return value can still make `read()` *block* (a delay — no corruption, single blocking writer emits whole records). NOTE: the sharper *shared-channel corruption* facet was split out and fixed as B10 (oversized `{shared}` values are now dropped, not partially written). What remains for B4 is purely making `read()` truly non-blocking at any size: make the results fh `O_NONBLOCK`, buffer partial records in `$self` across calls, WITH tests (a >4KB framed return value must not block `read()`).
