...
> **NEXT ACTION:** V32 — **Full leak/behaviour gate**: build with debug and run every custom-XS path (serialGets/spiDataRW/interrupts/physPinToWpi) under `valgrind --leak-check=full`; confirm no leaks/UB. This is the Phase 4 exit. Note device-gated success paths can't fully run here (serial: no device; SPI: wiringPi wants /dev/spidev0.0, Pi5 has spidev10.0 → B11) — gate the runnable paths (validation/croak/marshalling, interrupts via t/75, phys bounds) and document the residue. Then **V36** (final regression gate).
> **LAST SESSION (2026-06-05):** Ran **V31 — `physPinToWpi` (F21) — PASS**. Added a bounds guard to the XS `physPinToWpi` (returns the map's -1 "no such pin" sentinel for index <0 or >=64, via `sizeof` so it tracks the array) and mirrored it in the Perl `phys_to_wpi` wrapper (also -1 for undef/non-integer). New hardware-free `t/80-phys_to_wpi_bounds.t` (10 asserts) + MANIFEST; valgrind 0 errors / no invalid read on 1000 OOB calls; `make test` **240 PASS**; podchecker clean; added a Returns note to the POD. Downstream: only `RPi::WiringPi::Util.pm:39` uses `phys_to_wpi` and only with 0..63 (in-range) → no change. ⚠ consumer-facing: OOB input now returns -1 not garbage. Earlier same day: **V29+V30 PASS by ref**; **V28 PASS by ref**; **V27 spiDataRW PASS** (B11); **V26 serialGets PASS** (+RPi::Serial); **V33 PASS**; **V25 Phase 3 gate PASS**; **V13 PASS by ref**. **Phase 4 now down to just V32 (the valgrind gate).** Standing flags: V32 then V36; B5/B7/B9/B10/B11; commits pending across wiringpi-api/rpi-wiringpi/rpi-pin/rpi-serial; rpi-wiringpi cleanup under `refactor-setup-modes.md` V2-V9.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1-V31, V33-V35 archived)
