...
| ID | What | Command (run on the Pi) | Expected | Actual |
|----|------|-------------------------|----------|--------|
| V33 | Downstream gate (depends on V25): install the upgraded WiringPi::API on the Pi, then run RPi::WiringPi's `t/` suite plus the targeted exercisers for changed calls (see the **Downstream verification** section). Confirm valid calls behave identically, `shift_reg_setup` now croaks on out-of-range, and `serial_gets` no longer corrupts memory; then bump RPi::WiringPi's `Makefile.PL` prereq 2.3616→3.1801 and commit it **in that repo** | `cd ~/repos/rpi-wiringpi && prove -Ilib t/` (+ build_testing exercisers) | suite green for the wired hardware; behaviors confirmed; downstream prereq bumped | ⏳ **Partly run via isr-migration.md V9 (2026-06-04): the interrupt path is downstream-BREAKING — `RPi::Pin::set_interrupt` passes a string handler (new API needs a CODE ref) and the new model no longer auto-fires (needs `wait_interrupts`). User decision: keep WiringPi::API as-is, fix downstream (RPi::Pin not checked out here). See isr-migration.md V9 row for the full downstream change list. Also: installed module hits the executable-stack load error — must resolve before a real install. Non-interrupt calls (shift_reg/serial/i2c) still need their pass.** |
