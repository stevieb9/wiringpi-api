...
> **NEXT ACTION:** V36 — **Final regression gate** (run LAST, after every other task): clean `make realclean && perl Makefile.PL && make` (no warnings), `PI_BOARD=1 make test` (all hardware paths), podchecker (API.pm + INTERRUPTS.pod), live board sanity; confirm Changes still current for anything landed after V25. **Phase 4 is now complete (V32 PASS).**
> **LAST SESSION (2026-06-05):** Ran **V32 — Full leak/behaviour gate — PASS**. Built debug (`OPTIMIZE='-g -O0'`); ran every *runnable* custom-XS path under `valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=99`: new hardware-free `testing/valgrind_xs.pl` (200× each: spiDataRW marshalling/croak + Safefree-on-error, serialGets buffer loop via pipe fds incl. partial/EOF/zero/bad-fd, physPinToWpi OOB+valid) → **definitely lost: 0, indirectly lost: 0, 0 UB errors**; `t/75-interrupts.t` (45 ok) and `t/80-phys_to_wpi_bounds.t` (10 ok) likewise **0 lost / 0 UB**. All `API.c` frames in the logs are *still-reachable* boot-time XSUB registrations (`boot_WiringPi__API`, API.c:2369-2412) — no leak frame in any lost record across all three logs; the rest are Perl's own interpreter-arena allocations. **Residue documented (not a gap):** device *success* paths can't run here — serial (no device), SPI (wiringPi wants /dev/spidev0.0; Pi5/RP1 exposes spidev10.0 → **B11**); and helgrind has no custom-thread race surface off-hardware because `wiringPiISR2` (the only pthread spawn, via `_arm_interrupt`) is device-gated/faked in t/75. Added `testing/valgrind_xs.pl` to MANIFEST. Standing flags: **V36 only** now; B5/B7/B9/B10/B11; commits pending across wiringpi-api/rpi-wiringpi/rpi-pin/rpi-serial; rpi-wiringpi cleanup under `refactor-setup-modes.md` V2-V9.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1-V35 archived; only V36 remains)
