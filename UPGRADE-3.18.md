...
|----|------|---------|----------|--------|
| V1 | **Board-map precheck (do this first):** confirm `physPinToWpi()` (custom `phys_wpi_map`, API.xs:261 / API.h:37) and `physPinToGpio()` (wiringPi-native, wiringPi.h:282 — the upstream header itself cautions *not to use it outside pins 0-63 or on RP1*) map correctly **regardless of board/model** (esp. Pi5/RP1) — the early gate for **F9**. If either mapping is wrong, or correctness can't be established (unknown/unverifiable), **raise a new F# (next free) and resolve it before carrying on** — do **not** defer it to V24/V31. Then: Baseline build against 3.18 on the Pi — see exactly what currently compiles, links or fails, and capture the errors as the concrete Phase 1 worklist; explicitly scan the link/load output for an undefined `interruptHandler` symbol (F22) | `perl Makefile.PL && make 2>&1 \| tee /tmp/wpi-baseline.log` | `physPinToWpi()`/`physPinToGpio()` confirmed mapping correctly for this board (or a blocking F# raised); build runs to completion or fails with errors recorded to drive V2-V6 | ⏳ |
