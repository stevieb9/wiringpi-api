...
> **NEXT ACTION:** V28 — **Interrupt thread-safety (F17/F18)**: this targets the old dispatcher reading `perl_callbacks[]`/`SvRV` unlocked while `setInterrupt` replaces it. **The self-pipe ISR migration already DISSOLVED this** (no cross-thread SV table, no dispatcher thread) — so V28 closes mostly by reference: confirm in-tree that `perl_callbacks[]`/the dispatcher are gone and callbacks run only in the consuming interpreter. Per user: still sweep RPi::WiringPi (+ children) + FAQ.pod for any needed change (expected: none — interrupt downstream already handled in V33/isr-migration V9).
> **LAST SESSION (2026-06-05):** Ran **V27 — `spiDataRW` (F14/F15/F16) — PASS**. Rewrote the SPI marshalling glue as one idiomatic PPCODE XSUB: NULL-checks `av_fetch` (sparse/undef aref now croaks not crashes — F14), single `SvNV` per byte (F15), dropped the `inline_stack_*`/`dXSARGS`-in-plain-C/`PL_markstack_ptr` juggling (F16); contract preserved (`spiDataRW($chan,$aref,$len)`→list). Verified: all validation paths croak; valgrind 0 lost / no invalid r/w; `make test` 230 PASS. Success/round-trip path NOT runnable here (wiringPi opens `/dev/spidev0.0`; Pi5 only has spidev10.0) → **B11**. Downstream: **RPi::SPI** (cloned, `~/repos/rpi-spi`) is a pure consumer of `spiDataRW`, still binds + `t/00-load` PASS, no code change needed; RPi::WiringPi SPI → RPi::SPI (FAQ delegates). Earlier same day: **V26 serialGets PASS** (+ RPi::Serial twin fix); **V33 PASS**; **V25 Phase 3 gate PASS**; **V13 PASS by ref**. **Phase 4 hold lifted.** Standing flags: V31 real work; V28/V29/V30 close mostly by reference (dissolved by ISR migration); V32 final valgrind gate; then V36; B5/B7/B9/B10/B11; commits pending across wiringpi-api/rpi-wiringpi/rpi-pin/rpi-serial; rpi-wiringpi cleanup under `refactor-setup-modes.md` V2-V9.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1-V27, V33-V35 archived)
