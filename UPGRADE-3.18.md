...
> **NEXT ACTION:** V34 (NOT V7) — remove unsupported setup modes (`setup_sys`/`setup_phys` + `wiringPiSetupSys`/`wiringPiSetupPhys`), BREAKING. **Must also make the coordinated downstream edit in rpi-wiringpi (drop the `/^p/` phys branch, `RPi/WiringPi.pm:51-54`) alongside it — tracked by rpi-wiringpi's `refactor-setup-modes.md` (V1/V4).**
> **LAST SESSION (2026-06-04):** Ran **V6 on `rpi1` — PASS**. Bumped `$VERSION` 2.3617→3.1801, POD/README "2.36+"→"3.18+", copyright→"2017-2026" (consistent across pm/README/h/xs). Rebuilt so XS_VERSION matches (XSLoader bootstrap confirmed OK); no stale version refs remain. Prior: V1-V5 PASS. `perl -c` OK; podchecker only pre-existing B8; suite green (54 tests). Standing flags: V34 breaks RPi::WiringPi (downstream tracked under V33 + rpi-wiringpi's `refactor-setup-modes.md`); Phase 4 V26-V32 ⏸ HOLD; pre-existing POD errors tracked as B8.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1-V6 archived)
