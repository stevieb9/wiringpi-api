...
> **NEXT ACTION:** V2 — fix camelCase export mismatches in `@wpi_c_functions` (`wpiToGpio`→`wpiPinToGpio`, `lcdDefChar`→`lcdCharDef`, `lcdPutChar`→`lcdPutchar`)
> **LAST SESSION (2026-06-04):** Ran **V1 on `rpi1` (Pi5/RP1) — PASS**. Baseline build clean (exit 0, no warnings); `.so` loads. F9 board-map gate cleared via a new committed test `t/20-board_map_precheck.t` (added to MANIFEST): it cross-checks the custom `phys_wpi_map` against wiringPi 3.18's own tables and all 28 mapped header pins agree on this Pi5/RP1 — no blocking F# needed. F22 reproduced: `interruptHandler` is an unresolved symbol in the `.so`, calling it dies (already → V23). Note the build still compiled with the stale `-DVERSION=2.3617` (→V6). Standing flags: V34 breaks RPi::WiringPi (downstream tracked under V33 + rpi-wiringpi's `refactor-setup-modes.md`); Phase 4 V26-V32 ⏸ HOLD.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1 archived)
