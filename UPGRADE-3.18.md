...
> **NEXT ACTION:** V27 — **`spiDataRW` (F14/F15/F16)**: NULL-check `av_fetch` before deref (sparse/undef element → crash); compute `SvNV(*elem)` once; drop the VLA for `Newx`/`Safefree`; `croak` not `exit(1)`; replace the `dXSARGS`-in-plain-C + `PL_markstack_ptr` juggling with an idiomatic PPCODE XSUB. Per user: also check RPi::WiringPi (+ children, e.g. RPi::SPI — clone if absent) for matching changes, and validate FAQ.pod transfer.
> **LAST SESSION (2026-06-05):** Ran **V26 — `serialGets` (F13, CRITICAL) — PASS**. Rewrote `serialGets` as a self-allocating PPCODE XSUB (kills the heap overflow); `serial_gets` is now binary-safe (no `unpack "A*"`), validates args, reads up-to-nbytes honoring the port's VTIME (XSUB clears `O_NONBLOCK` since wiringPi's serialOpen sets it, defeating VTIME + making the old loop croak on EAGAIN), croaks not `exit(-1)`. Verified via pipe exercises (no serial HW): full read preserves NUL+space, partial-on-EOF, croaks on bad input; valgrind 0 lost / no invalid r/w; `make test` 230 PASS; added serial_gets POD. **Fixed the identical bug in the independent RPi::Serial** (`~/repos/rpi-serial`: tty_gets/gets, was `exit(-1)`) + POD + Changes 3.02 UNREL + VERSION bump; RPi::WiringPi reaches serial only via RPi::Serial (FAQ delegates, no transfer). Earlier same day: **V33 PASS** (downstream gate; exec-stack blocker resolved); **V25 Phase 3 gate PASS**; **V13 PASS by ref**. **Phase 4 hold lifted (user).** Standing flags: V27/V31 real work, V28/V29/V30 close mostly by reference (dissolved by ISR migration), V32 final valgrind gate, then V36; B5/B7/B9/B10; commits pending across wiringpi-api/rpi-wiringpi/rpi-pin/rpi-serial; rpi-wiringpi cleanup under `refactor-setup-modes.md` V2-V9.
> **ARCHIVE:** See UPGRADE-3.18-archive.md for completed V tasks (V1-V26, V33-V35 archived)
