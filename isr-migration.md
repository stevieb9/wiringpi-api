...
> **NEXT ACTION:** V5 — Perl-side dispatch: `set_interrupt($pin,$edge,$cb)` stores `$cb` in a lexical `%_interrupt_cb` (per-interpreter) then calls `_arm_interrupt` (already wired — just add the store). Add `dispatch_interrupts()` (non-blocking `sysread` of all 16-byte records, `unpack "i i q"`, call `$_interrupt_cb{$pin}->($edge,$ts_us)`; drain contract: `undef`→EINTR-retry else stop, `0`/EOF→terminal, never busy-spin) and `wait_interrupts($timeout_ms)` (`select` then dispatch). Expose `interrupt_dropped()` (the `interrupts_dropped` counter is already in API.xs — just add the accessor + XSUB).
> **LAST SESSION:** **2026-06-04.** Ran **V4 on `rpi1` — PASS** (the big rewrite): replaced the whole dispatcher/trampoline/`mine` apparatus with the self-pipe core — `isr2_writer` (pin from `userdata`, async-safe `write()`, atomic drop counter), lazy non-blocking `interrupt_pipe`, `_arm_interrupt` (stop-first-then-`wiringPiISR2`, kills F25), `interrupt_fd()`. Deleted 40 trampolines/`interrupt_handlers[]`/`perl_callbacks[]`/`event_queue`+mutex+cond+dispatcher/`mine`, XS `setInterrupt` (+export+decl), and (user-authorized) the dead `initThread`/`thread_callback_sv`/`PI_THREAD` — **this also completes threads-patch.md V1**. `set_interrupt` now arms via `_arm_interrupt($pin,$edge,0)` (callback store = V5). Residue grep empty; XS parses; `perl -c` OK; clean realclean+build (no warnings; ISR2/ISRStop resolve); `make test` PASS under bind-now; `interrupt_fd` exported (=-1 pre-arm). **Changes now kept current per-task** (user 2026-06-04): the `3.1801 UNREL` section already lists V2's `INT_EDGE_*` + `set_interrupt` validation, V3's `wiringPiISRStop`, and V4's interrupt rework + `interrupt_fd` + removed `setInterrupt`/`initThread` — earlier "deferred to V8" notes are superseded. Prior: V3 PASS (`wiringPiISRStop` wrap); V2 PASS (`INT_EDGE_*` + validation); V1 PASS. Drain contract for V5/V8: EINTR→retry, other-errno→remove+stop, EOF→remove+stop. `isr-examples-final.md` awaits commit.
> **ARCHIVE:** See isr-migration-archive.md for completed V tasks (V1-V4 archived)
