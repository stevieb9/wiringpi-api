...
> **NEXT ACTION:** Downstream interrupt migration in **RPi::Pin + RPi::WiringPi** (separate repos; RPi::Pin not checked out under ~/repos). Per V9's findings + the 2026-06-04 user decision (keep WiringPi::API explicit-dispatch, fix downstream): RPi::Pin `set_interrupt` must coerce a string handler to a coderef AND the stack must drive dispatch (background `wait_interrupts` loop / poll method), then update `t/200-202` + `defacto_interrupt.pl`, and wire `stop_interrupts()` into RPi::WiringPi `cleanup` (B8). Then re-run V9's gate. **Within this (wiringpi-api) repo the interrupt migration is done (V1-V8); no upstream change needed.** Optional remaining here: V10 (`background_interrupt` convenience), V11 (`auto_dispatch`/SIGIO) — additive, not required.
> **LAST SESSION:** **2026-06-04.** Ran **V9 gate — INCOMPATIBILITY FOUND** (not green). The new explicit-dispatch model breaks RPi::WiringPi: (1) `RPi::Pin::set_interrupt` passes a **string** handler → new `set_interrupt` croaks (needs CODE ref); (2) no auto-fire — nothing services the fd, so callbacks never run; `t/200`'s pull-then-assert pattern can't pass. Confirmed against the new module via `PERL5LIB=blib` (which also bypasses the installed-`.so` executable-stack error, V33). **User decision: keep WiringPi::API as-is; fix downstream** (primarily RPi::Pin, not checked out here; `rpi-wiringpi/lib` has no interrupt code). Findings + required downstream changes recorded in the V9 row + Discovery Tracking. Prior: V8 PASS (real-hardware full gate + `t/75-interrupts.t`, valgrind clean for the module); V7 PASS (POD + debounce + B8 POD fixes); V6 PASS (teardown); V5 PASS (dispatch); V4 PASS (self-pipe core, completed threads-patch.md V1); V3/V2/V1 PASS. **Changes kept current per-task**; `isr-examples-final.md` awaits commit; nothing committed yet (all in working tree). Drain contract for V5/V8: EINTR→retry, other-errno→remove+stop, EOF→remove+stop. `isr-examples-final.md` awaits commit.
> **ARCHIVE:** See isr-migration-archive.md for completed V tasks (V1-V8 archived)
...
|----|------|---------|----------|--------|
| V9 | **Downstream gate (RPi::WiringPi).** Install converted module; run `t/200-interrupt_rising_and_pud.t`, `t/201-…falling…`, `t/202-…both…`, `build_testing/build/defacto_interrupt.pl`. Externally-observable behavior unchanged (rising/falling/both fire). Note new dispatch model if a consumer must call `wait_interrupts`. | `cd ~/repos/rpi-wiringpi && prove -Ilib t/200-interrupt_rising_and_pud.t t/201*.t t/202*.t` (Pi) | interrupt suite green on wired hardware | ⏳ **2026-06-04: gate run — INCOMPATIBILITY FOUND, not green; upstream stays as-is, fix downstream (user decision). Two breaks (confirmed against new module via `PERL5LIB=blib`): (1) `RPi::Pin::set_interrupt` (RPi/Pin.pm:96) passes the handler **as a string** `'main::handler'` straight to `WiringPi::API::set_interrupt`, which now requires a CODE ref → croaks "requires \$callback to be a CODE reference". (2) New self-pipe model does **not auto-fire** — nothing calls `wait_interrupts`/`dispatch_interrupts`, so `RPi::Pin::set_interrupt` arms but callbacks never run; `t/200`'s pull-then-immediately-assert pattern can't pass. WiringPi::API itself needs NO change (per decision). Downstream work (separate repos — primarily **RPi::Pin, which is NOT checked out under ~/repos**; `rpi-wiringpi/lib` has no interrupt code): (a) RPi::Pin `set_interrupt` coerce a string handler to `\&{$name}` (no strict refs); (b) drive dispatch — RPi::WiringPi/RPi::Pin must run a dispatcher (background fork looping `wait_interrupts`, or expose a poll method), since the model is now explicit-dispatch; (c) update `t/200-202` + `defacto_interrupt.pl` to the dispatched model; (d) **B8**: wire `stop_interrupts()` into RPi::WiringPi `cleanup`. Note: installed RPi::Pin/WiringPi::API hit the **executable-stack** load error (V33 flag) — `PERL5LIB=blib` bypasses it; the install path still needs that resolved. Blocked here pending the downstream effort.** |
...
- **Found during V9 (downstream gate) — DOWNSTREAM-BREAKING, decision recorded.** The new explicit-dispatch interrupt model breaks RPi::WiringPi two ways: (1) `RPi::Pin::set_interrupt` passes a **string** handler that the new `set_interrupt` rejects (needs a CODE ref); (2) no auto-fire — callbacks only run when `wait_interrupts`/`dispatch_interrupts` services the fd, which RPi::Pin never calls. **User decision (2026-06-04): keep WiringPi::API as-is (explicit dispatch); fix downstream.** Full break-down + the required RPi::Pin/RPi::WiringPi changes are in the V9 row's Actual. Primary fix is in **RPi::Pin** (separate dist, not checked out under ~/repos). Also UPGRADE-3.18 V33 is the same gate — carries the same finding. Subsumes **B8** (RPi cleanup → `stop_interrupts()`).
