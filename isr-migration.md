...
B6: `auto_dispatch_interrupts` refinements — per-`set_interrupt` opt-in (vs the global switch), and a configurable delivery signal (default `SIGIO`, or a dedicated RT signal via `F_SETSIG`).
