...
> **NEXT ACTION:** V4 — Self-pipe core (F12/F10/F22): create the pipe at first arm; add one generic `isr2_writer(WPIWfiStatus, userdata)` that writes a fixed `{int pin; int edge; long long ts_us;}` record (pin from `userdata`, **not** `wfiStatus.pinBCM`); `_arm_interrupt(pin, edge, debounce)` calls `wiringPiISRStop(pin)` if re-arming then `wiringPiISR2(...)`; expose `interrupt_fd()`. **Delete** the 40 trampolines + `interrupt_handlers[]` + `MAKE_HANDLER`/`APPLY_TO_PINS`, the dead `interruptHandler()` + API.h decl + export, and `mine`/`perl_callbacks[]`/`event_queue`/mutex/cond/dispatcher.
> **LAST SESSION:** **2026-06-04.** Ran **V3 on `rpi1` — PASS**: thin XS wrap `wiringPiISRStop(pin)` added after `setInterrupt` (API.xs) + `@wpi_c_functions` export (→ `:wiringPi`/`:all`); legacy `waitForInterruptClose` left out. XS parses; `perl -c` OK; clean build (symbol resolves); verified importable + exported; calling pre-`setup()` hits wiringPi's own init abort (real symbol invoked). Changes deferred to V8. Prior: V2 PASS (`INT_EDGE_*` constants + ordered `set_interrupt` validation; discovery: reconcile `EDGE_RISING`→`INT_EDGE_*` in V7/V8); V1 PASS (header has ISR2/ISRStop/WPIWfiStatus, `void* userdata`). **V4 is the big rewrite** — design + code flow in ## Example + ## Thread-safety audit; V5/V8 drain contract = EINTR→retry, other-errno→remove+stop, EOF→remove+stop. `isr-examples-final.md` awaits commit.
> **ARCHIVE:** See isr-migration-archive.md for completed V tasks (V1-V3 archived)
