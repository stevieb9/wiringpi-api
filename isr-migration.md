...
> **NEXT ACTION:** V6 — Teardown: `stop_interrupt($pin)` = `wiringPiISRStop(pin)` + `delete $_interrupt_cb{$pin}`; `stop_interrupts()` = stop all armed pins, drain + close the pipe, reset state (incl. the cached `$_interrupt_fh`/`$_interrupt_fh_fd`). No thread to join. Add exports. (Needs an XS hook to close `interrupt_pipe[0..1]` and reset to -1 — coordinate with the Perl `_interrupt_fh` dup.)
> **LAST SESSION:** **2026-06-04.** Ran **V5 on `rpi1` — PASS**: Perl-side dispatch wired. `set_interrupt` stores `$cb` in the file-lexical `%_interrupt_cb`; added `dispatch_interrupts()` (non-blocking drain of 16-byte `i i q` records via a cached `<&` dup of `interrupt_fd()`, routes to per-pin callbacks; drain contract EINTR→retry/else-stop/EOF-stop, no busy-spin), `wait_interrupts($ms)` (4-arg select then dispatch), and XS `interrupt_dropped()` (F24 counter). Exports added. Verified hardware-free: 3 injected records route to the right pin-7/pin-9 callbacks (correct edge+ts), second drain=0, `wait_interrupts` works, dropped=0; clean build + `make test` PASS. Changes kept current. **Interrupts fire end-to-end again** (F17/F18/F19 stay dissolved — registry is in Perl). Prior: V4 PASS (self-pipe core; deleted dispatcher/trampolines/`mine` + `setInterrupt` + dead `initThread`, completing threads-patch.md V1); V3 PASS (`wiringPiISRStop`); V2 PASS (`INT_EDGE_*` + validation); V1 PASS. **Changes kept current per-task** (user 2026-06-04); `isr-examples-final.md` awaits commit; V7/V8 must reconcile `EDGE_RISING`→`INT_EDGE_*` in examples (Discovery Tracking). Drain contract for V5/V8: EINTR→retry, other-errno→remove+stop, EOF→remove+stop. `isr-examples-final.md` awaits commit.
> **ARCHIVE:** See isr-migration-archive.md for completed V tasks (V1-V5 archived)
